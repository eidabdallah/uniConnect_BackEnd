...
import { deleteUserSchema, getUsersSchema, registerAdminSchema, toggleStatusSchema, updateRoleSchema } from "./admin.validation.js";
const router = Router();
...
router.patch('/role/:id', asyncHandler(validation(toggleStatusSchema)), asyncHandler(auth(adminAndsuperAdmin)), asyncHandler(controller.toggleStatus));
router.patch('/:id', asyncHandler(validation(updateRoleSchema)), asyncHandler(auth(adminAndsuperAdmin)), asyncHandler(controller.updateRole));
